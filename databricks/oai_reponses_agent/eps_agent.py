...
LLM_ENDPOINT_NAME = os.environ.get("LLM_ENDPOINT", "databricks-gpt-5-2")
...
3. ONE TOOL CALL AT A TIME — make each search as a separate, individual function call
4. Execute tools FIRST, then synthesize results into a clear response

IMPORTANT: When calling multiple tools, call them ONE AT A TIME sequentially. 
Do NOT combine or batch tool arguments together.
</critical_rules>
...
- Tickets, issues, requests (Jira)
- Documentation and wikis (Confluence)
...
| Jira tickets, issues, requests | search_general_fallback | Status, assignee, priority |
| Confluence docs, wikis | search_general_fallback | Documentation, processes |
</tool_routing>
...
def glean_search(
    ...
    request_options = {
        "facetBucketSize": 100,
        # Note: returnLlmContentOverSnippets requires maxSnippetSize ≤ 10000
        # We use high maxSnippetSize (100k) to get full Gong transcripts instead
    }
    ...
    payload = {
        ...
        "maxSnippetSize": 100000,  # High limit to get full Gong transcripts
...
def _extract_content_text(content: list | str) -> str:
    """Extract text from Glean snippets or llmContent."""
    if not content:
        return ""
    if isinstance(content, str):
        return content
    if isinstance(content, list):
        # Concatenate all snippets - important for full Gong transcripts
        texts = []
        for item in content:
            if isinstance(item, dict):
                texts.append(item.get('text', item.get('snippet', '')))
...
def format_results(results: list[dict], source_name: str, query: str = "") -> str:
    ...
    for i, r in enumerate(results[:5], 1):
        ...
        content_text = _extract_content_text(r.get('content', []))
        
        # For Gong transcripts, allow more content (up to 8000 chars per result)
        # For other sources, keep it concise (1500 chars)
        max_len = 8000 if datasource == 'gong' else 1500
        
        entry = f"**[{i}] {title}**\n"
        entry += f"- **Datasource: {datasource}**\n"
        if len(content_text) > max_len:
            entry += f"- Content: {content_text[:max_len]}...\n"
        else:
            entry += f"- Content: {content_text}\n"
        entry += f"- URL: {url}\n"
    ...
    def call_llm(self, messages: list[dict], stream: bool = False) -> Any:
        ...
        with warnings.catch_warnings():
            ...
            return self.client.chat.completions.create(
                ...
                messages=messages,
                tools=TOOL_SPECS,
                parallel_tool_calls=False,  # Sequential calls to prevent JSON concatenation
                stream=stream,
            )
